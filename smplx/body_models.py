...
class SMPLLayer(SMPL):
    ...
    def forward(
        ...
        **kwargs,
        ...
        full_pose = torch.cat(
            [global_orient.reshape(-1, 1, 3, 3), body_pose.reshape(-1, self.NUM_BODY_JOINTS, 3, 3)], dim=1
        )
...
class SMPLH(SMPL):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if self.use_pca:
            ...
            self.register_buffer("right_hand_components", torch.tensor(right_hand_components, dtype=dtype))

        if self.flat_hand_mean or not hasattr(data_struct, "hands_meanl"):
            # left_hand_mean = np.zeros_like(data_struct.hands_meanl)
            left_hand_mean = np.zeros(15 * 3)
        else:
            left_hand_mean = data_struct.hands_meanl

        if self.flat_hand_mean or not hasattr(data_struct, "hands_meanr"):
            # right_hand_mean = np.zeros_like(data_struct.hands_meanr)
            right_hand_mean = np.zeros(15 * 3)
        else:
    ...
    def create_mean_pose(self, data_struct, flat_hand_mean=False):
        ...
        pose_mean = torch.cat([global_orient_mean, body_pose_mean, self.left_hand_mean, self.right_hand_mean], dim=0)
        return pose_mean
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if shapedirs.shape[-1] < self.SHAPE_SPACE_DIM + self.EXPRESSION_SPACE_DIM:
            print(
                f"WARNING: You are using a {self.name()} model, with only" " 10 shape and 10 expression coefficients."
            )
            expr_start_idx = 10
            expr_end_idx = 20
    ...
    def forward(
        ...
        **kwargs,
        ...
        if len(betas.shape) < 2:
            betas_ex = betas[None].expand(expression.shape[0], -1)
        else:
            betas_ex = betas

        model_vars = [betas_ex, global_orient, body_pose, transl, expression, left_hand_pose, right_hand_pose, jaw_pose]
        batch_size = 1
        for var in model_vars:
            if var is None:
                continue
        ...
        if betas_ex is None:
            betas_ex = torch.zeros([batch_size, self.num_betas], dtype=dtype, device=device)
        if transl is None:
            transl = torch.zeros([batch_size, 3], dtype=dtype, device=device)
        ...
        shape_components = torch.cat([betas_ex, expression], dim=-1)
...
class MANOLayer(MANO):
    def __init__(self, *args, **kwargs) -> None:
        ...
        super(MANOLayer, self).__init__(
            create_global_orient=False, create_hand_pose=False, create_betas=False, create_transl=False, *args, **kwargs
        )
