...
def build_edge_closure(
    ...
    if per_part:
        ...
        params_to_opt = [p for key, p in var_dict.items() if "pose" in key and p.requires_grad]
...
def run_fitting(
    exp_cfg, batch: Dict[str, Tensor], body_model: nn.Module, def_matrix: Tensor, mask_ids: Optional = None
) -> Dict[str, Tensor]:
    """Runs fitting"""
    vertices = batch["vertices"]  # flame vertices & faces, trimesh
    faces = batch["faces"]

    batch_size = len(vertices)
    dtype, device = vertices.dtype, vertices.device
    summary_steps = exp_cfg.get("summary_steps")  # default: 100
    interactive = exp_cfg.get("interactive")  # default: True

    # Get the parameters from the model
    skip_optim = exp_cfg.get("skip_optim", [])
    skip_optim = skip_optim.split(",")
    for i, item in enumerate(skip_optim):
        skip_optim[i] = item.strip()
    var_dict = get_variables(batch_size, body_model, skip_optim)  # learnable params
    ...
    if per_part:
        ...
        closure = build_edge_closure(body_model, var_dict, edge_loss, optimizer_dict, def_vertices, per_part=per_part)
    ...
    optimizer_dict = build_optimizer(list(var_dict.values()), optim_cfg)
    closure = build_vertex_closure(
        body_model, var_dict, optimizer_dict, def_vertices, vertex_loss=vertex_loss, per_part=False, mask_ids=mask_ids
    )
    minimize(
        optimizer_dict["optimizer"],
        closure,
