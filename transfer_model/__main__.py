...
def main() -> None:
    ...
    body_model = body_model.to(device=device)  # smplh -> smplx: 创建 SMPLX Model
    ...
    for ii, batch in enumerate(tqdm(dataloader)):
        ...
        for key in var_dict.keys():
            if isinstance(var_dict[key], np.ndarray):
                save_dict[key] = var_dict[key]
            else:
                save_dict[key] = var_dict[key].detach().cpu()
        ...
        for ii, path in enumerate(paths):
            ...
            output_path = osp.join(output_folder, f"{osp.splitext(fname)[0]}.npy")
            np.save(output_path, save_dict)

            output_path = osp.join(output_folder, f"{osp.splitext(fname)[0]}.obj")
            mesh = np_mesh_to_o3d(save_dict["vertices"][ii], save_dict["faces"])
